...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& object_in_camera_frame) {
    ...
    camera_in_global_frame.position.x = 1.0;  // Example camera position in global frame
    camera_in_global_frame.position.y = -0.15;
    camera_in_global_frame.position.z = 0.2;
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber sub = nh.subscribe("dope/pose_dex_cube_instanceable", 10, poseCallback);
    pose_pub = nh.advertise<geometry_msgs::PoseStamped>("object_pose", 10);
